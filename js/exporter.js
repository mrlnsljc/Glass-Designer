...
import { quote, money, ftIn, sqft } from './pricing.js';
...
export function printReport(project, renderDataURL) {
  ...
  const rows = q.rows.map((r) => {
    ...
    return `<tr>
      ...
      <td class="c">${sqft(r.panel.width, r.panel.height).toFixed(2)}</td>
      <td>${gt.name}</td>
      <td class="c">${r.panel.holes || 0}</td>
    ...
    *{box-sizing:border-box}
    ...
    <table class="cut">
      <thead><tr>
        ...
        <th class="c">Sq Ft</th><th>Glass</th><th class="c">Holes</th><th class="r">Cost</th>
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>
    <table class="totals">
      ...
      <tr><td>Drilled holes</td><td class="r">${q.totalHoles}</td></tr>
      <tr><td>Subtotal</td><td class="r">${money(q.subtotal)}</td></tr>
